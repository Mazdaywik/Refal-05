...
* **Расширяемость** — можно писать собственные функции на Си и вызывать их
  из Рефала. Размер макроцифр можно настраивать — см. [руководство][doc].
...
`PZ Oct 29 2004` или выше либо Рефал-5λ, набор библиотек для Рефала-5
[refal-5-framework][r5fw], а также компилятор языка Си89.

Рефал-5 VERSION-PZ или Version ПѮ можно скачать по ссылке:

<http://www.botik.ru/pub/local/scp/refal5/>

Для раскрутки при помощи Рефала-5λ вызывайте скрипт `bootstrap.cmd`
или `bootstrap.sh` с параметром `lambda`.
...
* Запустите файл `bootstrap.cmd`. Cкрипт развёртки создаст файл
  `c-plus-plus.conf.cmd` и сообщит о том, что не указан компилятор языка Си.
* Отредактируйте файл `c-plus-plus.conf.cmd` (да, я его скопировал у Рефала-5λ
  и забыл переименовать), указав в нём командную строку для запуска компилятора
  (см. инструкции в самом файле).
* Запустите ещё раз `bootstrap.cmd` — компилятор должен успешно собраться
...
* Установите переменную среды `R05PATH`, указав в ней через _точку с запятой_
  (Windows) или _двоеточие_ (unix-like) путь к папке `lib` этого каталога.
  Например:
  ```
...
```
refal05c hello refal05rts refal05bif
```
...
и `refal05bif` — библиотека со встроенными функциями Рефала (той же `Prout`).
...
* [Рефал-05-Unicode](https://github.com/slavenskoj/Refal-05-Unicode) — форк
  Данслава Славенского с полной поддержкой Юникода через библиотеку ICU
