/* Automatically generated file. Don't edit! */
#include "refalrts.h"


extern enum r05_fnresult r05c_Compare(struct r05_node *arg_begin, struct r05_node *arg_end);
extern enum r05_fnresult r05c_Ord(struct r05_node *arg_begin, struct r05_node *arg_end);
extern enum r05_fnresult r05c_Div(struct r05_node *arg_begin, struct r05_node *arg_end);
extern enum r05_fnresult r05c_Mod(struct r05_node *arg_begin, struct r05_node *arg_end);
static enum r05_fnresult r05c_OctDigit(struct r05_node *arg_begin, struct r05_node *arg_end);
extern enum r05_fnresult r05c_EscapeChar(struct r05_node *arg_begin, struct r05_node *arg_end);
static enum r05_fnresult r05c_EscapeChar_Aux(struct r05_node *arg_begin, struct r05_node *arg_end);
static enum r05_fnresult r05c_EscapeChar_SwCompare(struct r05_node *arg_begin, struct r05_node *arg_end);

static enum r05_fnresult r05c_OctDigit(struct r05_node *arg_begin, struct r05_node *arg_end) {
  r05_this_is_generated_function();
  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* 0 */
    if (! r05_number_left(0UL, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('0');
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* 1 */
    if (! r05_number_left(1UL, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('1');
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* 2 */
    if (! r05_number_left(2UL, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('2');
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* 3 */
    if (! r05_number_left(3UL, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('3');
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* 4 */
    if (! r05_number_left(4UL, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('4');
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* 5 */
    if (! r05_number_left(5UL, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('5');
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* 6 */
    if (! r05_number_left(6UL, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('6');
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* 7 */
    if (! r05_number_left(7UL, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('7');
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  return (enum r05_fnresult)(
    R05_RECOGNITION_IMPOSSIBLE | (__LINE__ << 8)
  );
}

enum r05_fnresult r05c_EscapeChar(struct r05_node *arg_begin, struct r05_node *arg_end) {
  r05_this_is_generated_function();
  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* '\n' */
    if (! r05_char_left('\n', &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_chars("\\n", 2);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* '\t' */
    if (! r05_char_left('\t', &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_chars("\\t", 2);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* '\r' */
    if (! r05_char_left('\r', &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_chars("\\r", 2);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* '\'' */
    if (! r05_char_left('\'', &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_chars("\\\'", 2);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* '\"' */
    if (! r05_char_left('\"', &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_chars("\\\"", 2);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* '\\' */
    if (! r05_char_left('\\', &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_chars("\\\\", 2);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *sOther_1;
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    struct r05_node *n[4] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* s.Other */
    if (! r05_svar_left(&sOther_1, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_open_call(n[0]);
    r05_alloc_function(r05c_EscapeChar_Aux, "EscapeChar_Aux");
    r05_alloc_svar(sOther_1);
    r05_alloc_open_call(n[1]);
    r05_alloc_function(r05c_Ord, "Ord");
    r05_alloc_svar(sOther_1);
    r05_alloc_close_call(n[2]);
    r05_alloc_close_call(n[3]);
    r05_push_stack(n[3]);
    r05_push_stack(n[0]);
    r05_push_stack(n[2]);
    r05_push_stack(n[1]);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  return (enum r05_fnresult)(
    R05_RECOGNITION_IMPOSSIBLE | (__LINE__ << 8)
  );
}

static enum r05_fnresult r05c_EscapeChar_Aux(struct r05_node *arg_begin, struct r05_node *arg_end) {
  r05_this_is_generated_function();
  do {
    struct r05_node *sOther_1;
    struct r05_node *sCode_1;
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    struct r05_node *n[6] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* s.Other s.Code */
    if (! r05_svar_left(&sOther_1, &bb[0], &be[0]))
      break;
    if (! r05_svar_left(&sCode_1, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_open_call(n[0]);
    r05_alloc_function(r05c_EscapeChar_SwCompare, "EscapeChar_SwCompare");
    r05_alloc_svar(sOther_1);
    r05_alloc_open_call(n[1]);
    r05_alloc_function(r05c_Compare, "Compare");
    r05_alloc_number(31UL);
    r05_alloc_svar(sCode_1);
    r05_alloc_close_call(n[2]);
    r05_alloc_open_call(n[3]);
    r05_alloc_function(r05c_Compare, "Compare");
    r05_alloc_svar(sCode_1);
    r05_alloc_number(128UL);
    r05_alloc_close_call(n[4]);
    r05_alloc_svar(sCode_1);
    r05_alloc_close_call(n[5]);
    r05_push_stack(n[5]);
    r05_push_stack(n[0]);
    r05_push_stack(n[4]);
    r05_push_stack(n[3]);
    r05_push_stack(n[2]);
    r05_push_stack(n[1]);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  return (enum r05_fnresult)(
    R05_RECOGNITION_IMPOSSIBLE | (__LINE__ << 8)
  );
}

static enum r05_fnresult r05c_EscapeChar_SwCompare(struct r05_node *arg_begin, struct r05_node *arg_end) {
  r05_this_is_generated_function();
  do {
    struct r05_node *sOther_1;
    struct r05_node *sCode_1;
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* s.Other '--' s.Code */
    if (! r05_svar_left(&sOther_1, &bb[0], &be[0]))
      break;
    if (! r05_char_left('-', &bb[0], &be[0]))
      break;
    if (! r05_char_left('-', &bb[0], &be[0]))
      break;
    if (! r05_svar_left(&sCode_1, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_svar(sOther_1);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  do {
    struct r05_node *sOther_1;
    struct r05_node *sCompare1_1;
    struct r05_node *sCompare2_1;
    struct r05_node *sCode_1;
    struct r05_node *bb[1] = { 0 };
    struct r05_node *be[1] = { 0 };
    struct r05_node *n[14] = { 0 };
    r05_start_sentence();
    r05_prepare_argument(&bb[0], &be[0], arg_begin, arg_end);
    /* s.Other s.Compare1 s.Compare2 s.Code */
    if (! r05_svar_left(&sOther_1, &bb[0], &be[0]))
      break;
    if (! r05_svar_left(&sCompare1_1, &bb[0], &be[0]))
      break;
    if (! r05_svar_left(&sCompare2_1, &bb[0], &be[0]))
      break;
    if (! r05_svar_left(&sCode_1, &bb[0], &be[0]))
      break;
    if (! r05_empty_seq(bb[0], be[0]))
      break;

    r05_reset_allocator();
    r05_alloc_char('\\');
    r05_alloc_open_call(n[0]);
    r05_alloc_function(r05c_OctDigit, "OctDigit");
    r05_alloc_open_call(n[1]);
    r05_alloc_function(r05c_Div, "Div");
    r05_alloc_svar(sCode_1);
    r05_alloc_number(64UL);
    r05_alloc_close_call(n[2]);
    r05_alloc_close_call(n[3]);
    r05_alloc_open_call(n[4]);
    r05_alloc_function(r05c_OctDigit, "OctDigit");
    r05_alloc_open_call(n[5]);
    r05_alloc_function(r05c_Mod, "Mod");
    r05_alloc_open_call(n[6]);
    r05_alloc_function(r05c_Div, "Div");
    r05_alloc_svar(sCode_1);
    r05_alloc_number(8UL);
    r05_alloc_close_call(n[7]);
    r05_alloc_number(8UL);
    r05_alloc_close_call(n[8]);
    r05_alloc_close_call(n[9]);
    r05_alloc_open_call(n[10]);
    r05_alloc_function(r05c_OctDigit, "OctDigit");
    r05_alloc_open_call(n[11]);
    r05_alloc_function(r05c_Mod, "Mod");
    r05_alloc_svar(sCode_1);
    r05_alloc_number(8UL);
    r05_alloc_close_call(n[12]);
    r05_alloc_close_call(n[13]);
    r05_push_stack(n[13]);
    r05_push_stack(n[10]);
    r05_push_stack(n[12]);
    r05_push_stack(n[11]);
    r05_push_stack(n[9]);
    r05_push_stack(n[4]);
    r05_push_stack(n[8]);
    r05_push_stack(n[5]);
    r05_push_stack(n[7]);
    r05_push_stack(n[6]);
    r05_push_stack(n[3]);
    r05_push_stack(n[0]);
    r05_push_stack(n[2]);
    r05_push_stack(n[1]);
    r05_splice_from_freelist(arg_begin);
    r05_splice_to_freelist(arg_begin, arg_end);
    return R05_SUCCESS;
  } while ( 0 );

  return (enum r05_fnresult)(
    R05_RECOGNITION_IMPOSSIBLE | (__LINE__ << 8)
  );
}


/* End of file */
