...
bool refalrts::function_match(
  refalrts::RefalFunctionPtr fn, refalrts::Iter pos
) {
  return ( cDataFunction == pos->tag ) && ( pos->function_info.ptr == fn);
}
...
bool refalrts::char_match(
  char ch, refalrts::Iter pos
) {
  return ( cDataChar == pos->tag ) && ( pos->char_info == ch);
}
...
bool refalrts::number_match(
  refalrts::RefalNumber num, refalrts::Iter pos
) {
  return ( cDataNumber == pos->tag ) && ( pos->number_info == num);
}
...
bool refalrts::ident_match(
  refalrts::RefalIdentifier ident, refalrts::Iter pos
) {
  return ( cDataIdentifier == pos->tag ) && ( pos->ident_info == ident);
}
...
bool refalrts::brackets_match(
  refalrts::Iter& right, refalrts::Iter left /* pos */
) {
  if( cDataOpenBracket != left->tag ) {
    return false;
  } else {
    right = left->link_info;
    return true;
  }
}
...
bool refalrts::adt_match(
  refalrts::Iter& right, refalrts::Iter left /* pos */
) {
  if( cDataOpenADT != left->tag ) {
    return false;
  } else {
    right = left->link_info;
    return true;
