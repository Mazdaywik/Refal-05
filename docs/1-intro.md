...
**Что нам это даёт?** Функция `Mu` в Рефале-05 определена как

    $ENTRY Mu {
      s.Func e.Arg = <s.Func e.Arg>;
    }

Да, именно [так и определена][MuSrc]. Поэтому пример выше с `mylibrary.ref`
...
`Library.ref`. Без файлов рантайма просто ничего не скомпилируется, без
...
* **`R05-CompilerUtils`** — предоставляет вспомогательные средства работы
  компилятора: поиск исходных файлов и вызов компилятора языка Си. Зависит
  от `LibraryEx`.
* **`R05-Generator`** — компилирует абстрактное синтаксическое дерево в Си.
  ...
  компиляции в файл. Зависит от `R05-AST` и `LibraryEx`.
...
исходники компилятора (`src/*.ref`) и библиотек (`refalrts.*`, `Library.ref`)
...
* стиль должен быть логичным, единообразным и согласованным — для любого
  куска кода стиль должен подсказывать единственно верный способ его записи.
...
[MuSrc]: https://github.com/Mazdaywik/Refal-05/blob/master/lib/Library.ref#L9-L14
