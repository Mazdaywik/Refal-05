...
и библиотеке встроенных функций. Папка `src` опциональная — там находятся повторно
...
`R05-Generator.ref` в папке `src`. Будут созданы файлы `myprogram.c`,
`LibraryEx.c` и `R05-Generator.c` в текущей папке. Если задана
непустая переменная `R05CCOMP`, будет вызван компилятор Си, которому будут
переданы четыре сишных файла и ключи `-I` с путями к папкам `lib` и `src`.
