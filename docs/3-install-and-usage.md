...
После первого запуска `bootstrap.cmd` (если установлен Рефал-5) или
`bootstrap.cmd lambda` (если установлен Рефал-5λ) установка завершится
заведомо с ошибкой и будет создан файл `c-plus-plus.conf.cmd`. После этого
в него нужно вписать командную строку для запуска компилятора и вызвать
`bootstrap.cmd`/`bootstrap.cmd lambda` второй раз. В конфигурационном файле
...
* `R05PATH` — пути для поиска исходников, перечисляются через стандартный
  разделитель операционной системы: точку с запятой на Windows или двоеточие
  на unix-like.
  Ей нужно присвоить полные пути до каталога `lib` этого репозитория.

В переменной `R05PATH` нужно указать полный путь к папке `lib` данного
репозитория. Папку `lib` нужно указывать обязательно, чтобы компилятор мог
находить пути к файлам библиотеки поддержки времени выполнения («рантайм»)
и библиотеке встроенных функций.
...
добавить следующие строчки:
    ...
    set R05PATH=C:\Refal-05\lib

(Правда, собрать его придётся на какой-то другой ОС — скрипты сборки
не совместимы с Windows 98.)

Настройка окружения завершена. Компилятором теперь можно пользоваться.

### Настройка размера макроцифр (макросы `R05_NUMBER_‹тип›`)
У компилятора Рефала-05 можно конфигурировать целый тип данных, представляющий
значение макроцифры, установкой одного из следующих макросов:

* `R05_NUMBER_INT` — будет использоваться `unsigned int`. Это вариант
  по умолчанию, т.е. если никакой вариант не установлен, то данный макрос
  в `refal05rts.h` будет установлен автоматически.
* `R05_NUMBER_LONG` — будет использоваться `unsigned long`.
* `R05_NUMBER_LONGLONG` — будет использоваться тип `unsigned long long`.
  Данный тип можно использовать только в компиляторах, поддерживающих
  стандарт С99 и выше.
* `R05_NUMBER_UINT32_T`, `R05_NUMBER_UINT64_T` — использование, соответственно,
  типов `uint32_t` и `uint64_t`. Эти типы тоже требуют стандарт C99 и выше.
  При их использовании в файле `refal05rts.h` будут автоматически подключены
  заголовки `stdint.h` и `inttypes.h`.
* `R05_NUMBER_CUSTOM=…` — пользовательский тип (должен быть беззнаковым),
  макрос должен раскрываться в определение соответствующего типа.

Для каждого из перечисленных выше макросов, кроме последнего, автоматически
устанавливается макрос `PRIuR05`, соответствующий спецификатору беззнакового
целого для функций семейства `printf`, его использование аналогично другим
спецификаторам из `inttypes.h`.

    printf("Value is %" PRIuR05 "\n", value->info.number);

При использовании макроса `R05_NUMBER_CUSTOM=…` пользователь должен сам
задать соответствующее значение макроса `PRIuR05`, например, так (синтаксис
командной строки для Windows):

    set R05CFLAGS=-DR05_NUMBER_CUSTOM=size_t -DPRIuR05=\"zu\"

Чтобы компилятор поддерживал длинные макроцифры в исходных текстах,
пересоберите его с соответствующим макросом `R05_NUMBER_‹тип›`, например,
укажите его параметром в `c-plus-plus.conf.*`.

Подробнее о переменной `R05CCOMP` и `R05CFLAGS` написано в следующей главе.
...
который находится в текущей папке, поэтому текущая папка должна быть доступна
для записи.
...
Библиотека встроенных функций `refal05bif.c` содержит реализации всех
встроенных функций языка, написанные на Си.

Файлы `refal05rts.c` и `refal05bif.c`, а также `refal05rts.h`, содержащий
...
и `R05PATH`) и записана такая командная строка:
    ...
    refal05c myprogram refal05rts refal05bif LibraryEx

Компилятор в этом случае найдёт `myprogram.ref` в текущей папке, `refal05rts.c`
и `refal05bif.c` в папке `lib`, `LibraryEx.ref` в библиотеке «refal-5-framework».
Будут созданы файлы `myprogram.c` и `LibraryEx.c` в текущей папке. Если задана
непустая переменная `R05CCOMP`, будет вызван компилятор Си, которому будут
переданы четыре сишных файла и ключи `-I` с путём к папке `lib`.
...
рассмотрении [деталей реализации][4].
