...
Поддержка Unicode (Рефал-05-Unicode)
------------------------------------

Версия **Рефал-05-Unicode** включает полную поддержку Unicode через библиотеку
ICU (International Components for Unicode). Для использования Unicode-функций
необходимо:
...
**На Windows:**
Загрузите и установите ICU с [официального сайта](http://site.icu-project.org/).

### Сборка с поддержкой Unicode

При сборке компилятора используйте скрипт `bootstrap-unicode.sh` вместо обычного
`bootstrap.sh`. Компилятор автоматически определит наличие ICU и включит поддержку
Unicode.

### Дополнительные флаги компиляции

При использовании Unicode добавьте флаг `-DR05_UNICODE` в переменную `R05CCOMP`:

**На POSIX-системах:**
```bash
export R05CCOMP="gcc -DR05_POSIX -DR05_UNICODE -licuuc -licui18n"
```

**На Windows:**
```
set R05CCOMP=cl /EHcs /W3 /wd4996 /O2 /DR05_UNICODE /link icuuc.lib icuin.lib
```

### Возможности Unicode

* Поддержка всех символов Unicode в строках и символьных литералах
* Шестнадцатеричные литералы: `0x4E2D` для записи Unicode символов
* Расширенные функции `Chr`, `Ord`, `Type`, `Upper`, `Lower` для работы с Unicode
* Новые функции `ChrHex` и `OrdHex` для работы с шестнадцатеричными кодами
* Исходные файлы и ввод/вывод в кодировке UTF-8

Подробную документацию см. в файле `README-UNICODE.md`.
