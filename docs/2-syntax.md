...
### 16. Last
    ...
    <Last s.Len e.Items> == (e.Prefix) e.Suffix

    e.Items : e.Prefix e.Suffix
    |e.Suffix| == s.Len || { |e.Suffix| < s.Len && |e.Suffix| == 0 }

**Семантика.** Отделяет от строки суффикс указанной длины. Если строка
короче, то она вся возвращается как суффикс.
...
### 34. Sysfun
Функция `Sysfun` по исходной задумке предназначена для того, чтобы в неё
прятать недокументированные возможности Рефала-5. В частности, первая
системная функция предназначена для ускорения десериализации функцией
`Xxin` библиотеки `reflib` Рефала-5 VERSION PZ или Version Пξ.

#### Системная функция № 1

    <Sysfun 1 e.FileName> == e.AnyExpr
...
* переводы строк игнорируются везде, в том числе, внутри образов символов,
...
Символы-слова, изображающие встроенные функции (включая `"+"` и другие),
становятся нагруженными идентификаторами для соответствующих встроенных
функций.
...
#### Системная функция № 2
    ...
    <Sysfun 2 e.FileName (s.Width e.AnyExpr)> == пусто

    e.FileName ::= s.CHAR+
    s.Width ::= s.NUMBER

**Семантика:** сериализует произвольное объектное выражение, сохраняя его
в файле `e.FileName`, длины строчек ограничены `s.Width`. Формат сериализации
совместим с системной функцией № 1, обе функции проектировались как
взаимно-обратные. Перевод строки может рассекать образы символов в произвольном
месте.

**Совместимость с Рефалом-5:** отсутствует, однако в Рефале-5 присутствует
функция `DeSysfun` с аналогичной семантикой.

По мнению разработчика Рефала-05, наличие `DeSysfun` является ошибкой
проектирования, соответствующая функция или должна быть системной функцией № 2,
или обе функции не должны «прятаться» в недокументированную тень, например,
иметь имена `Serialize` и `Deserialize`.
...
значение записывается как литера `'-'` с последующим числом. Знак `'+'`
ни на что не влияет, добавлен для совместимости с Рефалом-5.
...
### 62. DeSysfun
    ...
    <DeSysfun e.FileName (s.Width e.AnyExpr)> == пусто

    e.FileName ::= s.CHAR+
    s.Width ::= s.NUMBER

**Семантика:** делает то же самое, что и системная функция № 2 (`<Sysfun 2
...>`), только выдаёт сообщение на `stderr` о том, что она нежелательная
(deprecated) в Рефале-05.

**Совместимость с Рефалом-5:** полная, за исключением выдачи сообщения
на `stderr`.
