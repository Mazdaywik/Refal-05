...
refalrts::FnResult FindFiles(refalrts::Iter arg_begin, refalrts::Iter arg_end) {
  do {
    refalrts::Iter elems[100];

    refalrts::Iter elems[0] = arg_begin;
    refalrts::Iter elems[1] = arg_end;
    refalrts::move_left( elems[0], elems[1] );
    refalrts::move_left( elems[0], be_0 );
    ...
    const static refalrts::ResultAction raa[] = {
      ...
      { cCmdName, 12, & Map, "Map" },
      { cCmdOpenBracket, 13 },
      { cCmdName, 14, & AnalyzeFile_ByFolders, "...." },
      { cCmdName, & ..., "..." },
      { cCmdSpliceEVar, eFolder_b_1, eFolder_e_1 },
      { cCmdCloseBracket },
      { cCmdSpliceEVar, eFile_b_1, eFile_b_2 },
      { cCmdCloseCall },
      { cCmdEnd }
    };

    refalrts::FnResult res = refalrts::interpret_array( raa, elems, 100, arg_begin, arg_end );
    return res;
#else
